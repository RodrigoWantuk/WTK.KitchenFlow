...
- **Status:** In Progress (PR #34 final residual remediation)
...
Implement residual remediations on draft PR #34 (definition retry, suggestion result status, HomeDisplayText, reduced-motion unknown fallback); await exact-head CI; mark Completed only when green.
