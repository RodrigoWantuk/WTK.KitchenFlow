...
The next expansion benchmark also validates the bounded `thumbnailVisual` descriptor added by PLAN-0017. The descriptor must remain provider-independent and must not contain pantry/private context or image-generation policy.
...
4. selected-recipe expansion with visual-descriptor fidelity and deterministic canonicalization.
