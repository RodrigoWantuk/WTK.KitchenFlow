...
export interface StoreActions {
  ...
  replaceMeal: (
    id: string,
    newRecipeId: string,
    opts?: { removePrevShopping?: boolean },
  ) => void;
  moveMeal: (
    id: string,
  ...
  sendItemsToShopping: (
    items: Array<
      Partial<ShoppingItem> & Pick<ShoppingItem, "name" | "qty" | "unit">
    >,
  ) => void;
  resetAll: () => void;
}
...
/** Preferences-only key for production — never stores authed or personal synthetic state. */
const LS_PREFS_KEY = "cocinaris_prefs_v1";

export interface StoreProviderOptions {
  children: ReactNode;
  /** When false, do not seed SEED_* fixtures or treat localStorage as authoritative personal state. */
  enablePrototypeFixtures?: boolean;
  /** When false, never persist or restore `authed` from localStorage. */
...
function loadPrefs(): Partial<Pick<StoreState, "lang" | "theme">> | null {
  try {
    const raw = localStorage.getItem(LS_PREFS_KEY);
    if (!raw) return null;
    return JSON.parse(raw) as Partial<Pick<StoreState, "lang" | "theme">>;
  } catch {
    return null;
  }
}

function persistPrototype(state: StoreState) {
  try {
...
function persistPrefs(state: StoreState) {
  try {
...
function buildInitialState(enablePrototypeFixtures: boolean): StoreState {
...
export function StoreProvider({
  ...
  const [state, setState] = useState<StoreState>(() => {
    const initial = buildInitialState(enablePrototypeFixtures);
    if (!enablePrototypeFixtures) {
      const prefs = loadPrefs();
      return prefs ? { ...initial, ...prefs, authed: false } : initial;
    }
    const persisted = loadState();
    if (!persisted) return initial;
    const merged = { ...initial, ...persisted };
    if (!persistPrototypeAuth) {
      merged.authed = false;
    }
    return merged;
  });
  ...
  useEffect(() => {
    if (enablePrototypeFixtures && persistPrototypeAuth) {
      persistPrototype(state);
      return;
    }
    if (enablePrototypeFixtures && !persistPrototypeAuth) {
      const { authed: _omitAuth, ...rest } = state;
      persistPrototype({ ...rest, authed: false } as StoreState);
      return;
    }
    persistPrefs(state);
  }, [state, enablePrototypeFixtures, persistPrototypeAuth]);
  useEffect(() => {
  ...
  const scenarioView = useMemo(() => {
    ...
    if (s === "attention")
      pantry = pantry.map((i) =>
        ["p5", "p6", "p8", "p12", "p16"].includes(i.id) ? i : i,
      );
    if (s === "missing")
      pantry = pantry.filter((i) => !["p10", "p11"].includes(i.id));
    if (s === "planned") plan = state.plan;
    if (s === "thaw")
      pantry = pantry.map((i) =>
        i.id === "p10" ? { ...i, notes: "Descongelando desde manhã" } : i,
      );
    if (s === "shoppingOngoing")
      shopping = shopping.map((it, idx) =>
        idx < 2 ? { ...it, checked: true } : it,
      );
    if (s === "cooking" || s === "cookingPaused")
      activity = {
        type: "cook",
        recipeId: "r1",
        step: 3,
        paused: s === "cookingPaused",
    ...
    if (s === "frozenPortions")
      pantry = [
        ...
        {
          id: "pF",
          name: "Sopa de feijão (porção)",
          category: "leftover",
          mode: "measured",
          qty: 2,
          unit: "porções",
          location: "freezer",
          notes: "Congelado após preparo",
        },
      ];
    if (s === "smartOffline") smartAvailable = false;
    if (s === "serviceError") {
      smartAvailable = false;
      error = t(state.lang, "states.error");
    }
    if (s === "insufficient") {
      /* keep pantry small */ pantry = pantry.slice(0, 3);
    }
    if (s === "onboardingIncomplete") {
      /* handled via profile flag override */
    }
    if (s === "newUser") plan = [];
    ...
    if (s === "awaitingAcceptance") {
      plan = generateFullWeekPlan(allRecipeIds).map((p) => ({
        ...p,
        state: "suggested",
      }));
    }
    if (s === "manyDrafts") {
      plan = generateFullWeekPlan(allRecipeIds)
        .slice(0, 8)
        .map((p) => ({ ...p, state: "draft" }));
    }
    if (s === "listReady") {
      ...
      pantry = pantry.filter(
        (i) => !["p10", "p11", "p12", "p15"].includes(i.id),
      );
    }
        ...
        pantry = [
          {
            ...
            unit: "ml",
            location: "freezer",
            notes: "Preparado no domingo.",
            _isComponent: true,
            _uncertainty: "confirmed",
            reservedFor,
          },
          ...pantry,
        ];
  ...
  const recipesWithMatch = useMemo(() => {
    ...
    return RECIPES.map((r) => {
      const have = r.ingredients.filter(
        (i) => i.match && pantryIds.has(i.match),
      ).length;
  ...
  const actions = useMemo<StoreActions>(
    () => ({
      ...
      addPantry: (item) =>
        set((s) => ({
          pantry: [{ ...item, id: `p_${Date.now()}` }, ...s.pantry],
        })),
      updatePantry: (id, patch) =>
        set((s) => ({
          pantry: s.pantry.map((i) => (i.id === id ? { ...i, ...patch } : i)),
        })),
      removePantry: (id) =>
      ...
      addShopping: (item) =>
        set((s) => ({
          shopping: [
            { ...item, id: `s_${Date.now()}`, checked: false },
            ...s.shopping,
          ],
        })),
      toggleShopping: (id) =>
      ...
      addMissingToShopping: (recipeId) =>
        set((s) => {
          ...
          const additions: ShoppingItem[] = missing.map((m, idx) => ({
      ...
      startCooking: (recipeId) =>
        set({
          cooking: { recipeId, step: 0, paused: false, startedAt: Date.now() },
          activity: {
            type: "cook",
            recipeId,
            step: 0,
      ...
      updateCookStep: (step) =>
        set((s) => ({
          cooking: s.cooking ? { ...s.cooking, step } : null,
          activity: s.activity ? { ...s.activity, step } : null,
        })),
      pauseCooking: () =>
        set((s) => ({
          cooking: s.cooking ? { ...s.cooking, paused: true } : null,
          activity: s.activity ? { ...s.activity, paused: true } : null,
        })),
      resumeCooking: () =>
        set((s) => ({
          cooking: s.cooking ? { ...s.cooking, paused: false } : null,
          activity: s.activity ? { ...s.activity, paused: false } : null,
        })),
      finishCooking: (reconciliation) =>
        set((s) => ({
          cooking: null,
          activity: null,
          history: [
      ...
      toggleFavorite: (recipeId) =>
        set((s) => ({
          favorites: s.favorites.includes(recipeId)
            ? s.favorites.filter((i) => i !== recipeId)
      ...
      addPlan: (entry) =>
        set((s) => ({
          plan: [{ ...entry, id: `pl_${Date.now()}` }, ...s.plan],
        })),
      removePlan: (id) =>
      ...
      acceptSuggestion: (id) =>
        set((s) => ({
          plan: s.plan.map((p) =>
            p.id === id ? { ...p, state: "accepted" } : p,
          ),
        })),
      setMealState: (id, mealState) =>
        set((s) => ({
          plan: s.plan.map((p) =>
            p.id === id ? { ...p, state: mealState } : p,
          ),
        })),
      replaceMeal: (id, newRecipeId, opts = {}) =>
        set((s) => {
          const entry = s.plan.find((p) => p.id === id);
          if (!entry) return {};
          const prevRecipeId = entry.recipeId;
          const nextPlan = s.plan.map((p) =>
            p.id === id
              ? {
                  ...p,
                  recipeId: newRecipeId,
                  state: "accepted",
                  _replacedFrom: prevRecipeId,
                }
              : p,
          );
          let nextShopping = s.shopping;
          if (opts.removePrevShopping && prevRecipeId) {
            nextShopping = s.shopping.filter(
              (it) => !(it.origin === "recipe" && it.recipeId === prevRecipeId),
            );
          }
          return { plan: nextPlan, shopping: nextShopping };
        }),
      moveMeal: (id, target, resolution = "swap") =>
        set((s) => {
          const entry = s.plan.find((p) => p.id === id);
          if (!entry) return {};
          const occupying = s.plan.find(
            (p) =>
              p.id !== id && p.day === target.day && p.meal === target.meal,
          );
          let next = s.plan;
          if (
            !occupying ||
            ...
            next = s.plan.map((p) =>
              p.id === id ? { ...p, day: target.day, meal: target.meal } : p,
            );
          } else if (resolution === "swap") {
            next = s.plan.map((p) => {
              if (p.id === id)
                return { ...p, day: target.day, meal: target.meal };
              if (p.id === occupying.id)
                return { ...p, day: entry.day, meal: entry.meal };
              return p;
            });
          } else if (resolution === "replace") {
            next = s.plan
              .filter((p) => p.id !== occupying.id)
              .map((p) =>
                p.id === id ? { ...p, day: target.day, meal: target.meal } : p,
              );
          }
      ...
      changeMealSlot: (id, meal) =>
        set((s) => ({
          plan: s.plan.map((p) => (p.id === id ? { ...p, meal } : p)),
        })),
      markMealDone: (id, mode) =>
        set((s) => ({
          plan: s.plan.map((p) =>
            p.id === id
              ? mode === "skipped"
                ...
                : {
                    ...p,
                    state: "done",
                    _reconciliationPending: mode === "onlyDone",
                  }
              : p,
          ),
        })),
      clearReconcilePending: (id) =>
        set((s) => ({
          plan: s.plan.map((p) =>
            p.id === id ? { ...p, _reconciliationPending: false } : p,
          ),
        })),
      finishShopping: () =>
        set((s) => {
          ...
          return {
            shopping: remaining,
            pantry: [...additions, ...s.pantry],
            _lastReconciled: { moved: additions.length, at: Date.now() },
          };
        }),
      sendItemsToShopping: (items) =>
        set((s) => {
          const existingKey = (n: string | undefined, u: string | undefined) =>
            `${(n || "").toLowerCase()}|${(u || "").toLowerCase()}`;
          const existing = new Map(
            s.shopping.map((it) => [existingKey(it.name, it.unit), it]),
          );
          ...
          items.forEach((it) => {
            ...
            if (existing.has(k)) {
              ...
              const merged = {
                ...cur,
                qty: (Number(cur.qty) || 0) + (Number(it.qty) || 0),
              };
              ...
              const created: ShoppingItem = {
                ...
                checked: false,
                aisle: it.aisle || "grocery",
                origin: it.origin || "plan",
          ...
          return {
            shopping: [
              ...additions,
              ...merged.filter((it) => !additions.find((a) => a.id === it.id)),
            ],
          };
        }),
      resetAll: () => setState(buildInitialState(enablePrototypeFixtures)),
    }),
    [set, enablePrototypeFixtures],
  );
