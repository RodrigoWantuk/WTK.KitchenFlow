...
import { projectPreparationRoute } from "../../features/preparation-route/projectPreparationRoute";
 ...
 */
export class MockPreparationRouteRepository
  implements PreparationRouteRepository
{
  ...
  private snapshot: PreparationRouteProjection;
  ...
  constructor(
    ...
    this.snapshot = projectPreparationRoute(this);
  }
  ...
  markDone(taskId: string): void {
    ...
    this.inProgress = new Set(
      [...this.inProgress].filter((id) => id !== taskId),
    );
    this.refreshSnapshotAndEmit();
  }
  ...
  private refreshSnapshotAndEmit(): void {
    this.snapshot = projectPreparationRoute(this);
...
/**
 * Shared singleton for prototype composition roots and tests.
 * Production composition roots must not import or default to this instance.
 */
export const sharedMockPreparationRouteRepository =
  new MockPreparationRouteRepository();
