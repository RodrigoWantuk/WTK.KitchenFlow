  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-full max-w-md">
        <SheetHeader>
          <SheetTitle className="font-display text-2xl">{tr("scenarios.title")}</SheetTitle>
        </SheetHeader>
        <p className="mb-4 mt-1 text-sm text-muted-foreground">Alterne cenários para ver como as telas respondem.</p>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {SCENARIOS.map(s => (
            <Button
              ...
              onClick={() => { setScenario(s); setOpen(false); }}
