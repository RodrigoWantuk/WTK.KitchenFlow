...
import { Home, Package, BookOpen, CalendarDays, ShoppingBag, Settings as SettingsIcon, Sun, Moon, Globe } from "lucide-react";
...
function Brand() {
  return (
    <Link to="/app/hoje" data-testid="brand-link" className="flex items-center gap-2">
      <span className="grid h-9 w-9 place-items-center rounded-xl bg-primary text-primary-foreground font-display text-lg">C</span>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button data-testid="lang-switch" variant="ghost" size="sm" className="gap-1.5">
          <Globe className="h-4 w-4" /> {languages.find(l => l.code === lang)?.label}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end">
        {languages.map(l => (
          <DropdownMenuItem key={l.code} data-testid={`lang-opt-${l.code}`} onClick={() => setLang(l.code)}>{l.label}</DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
function ThemeToggle() {
  ...
  return (
    <Button data-testid="theme-toggle" variant="ghost" size="icon" onClick={() => setTheme(dark ? "light" : "dark")}>
...
export default function AppShell({ children }) {
  ...
  if (inCookMode) {
    return <div className="min-h-screen bg-background">{children}<Toaster /></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/85 backdrop-blur">
        ...
        {!smartAvailable && (
          <div data-testid="smart-offline-banner" className="border-t border-border bg-warning/15 px-4 py-2 text-center text-xs text-foreground/80 md:px-8">
            {tr("states.offline")}
          </div>
        )}
        {scenarioError && (
          <div data-testid="scenario-error-banner" className="border-t border-border bg-destructive/15 px-4 py-2 text-center text-xs text-destructive md:px-8">
            {scenarioError}
          </div>
        )}
      </header>
      ...
      <div className="mx-auto flex max-w-7xl gap-8 px-4 pb-28 pt-6 md:px-8 md:pb-10">
        ...
        <aside className="sticky top-24 hidden h-fit w-56 shrink-0 md:block">
          ...
          {activity && (
            <Link to={activity.type === "cook" ? `/app/cozinhar/${activity.recipeId}` : "/app/hoje"} data-testid="sidebar-activity" className="mt-6 block rounded-2xl border border-accent/30 bg-accent/10 p-3">
              <p className="font-display text-sm">{activity.paused ? "Preparo pausado" : "Em andamento"}</p>
