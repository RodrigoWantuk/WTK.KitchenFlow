...
export function RealityChangedDialog({ open, onOpenChange, onApply }: { open: boolean; onOpenChange: (o: boolean) => void; onApply?: (v: any) => void }) {
...
export function ScheduleComponentDialog({ open, onOpenChange, onConfirm }: { open: boolean; onOpenChange: (o: boolean) => void; onConfirm?: () => void }) {
...
export function RecipeComponentDependency({ onSchedule }: { onSchedule?: () => void }) {
...
/* ---------- Simulation Dialog ---------- */
export function SimulationDialog({ open, onOpenChange, onAccept }: { open: boolean; onOpenChange: (o: boolean) => void; onAccept?: (choice: string) => void }) {
  const [choice, setChoice] = useState<string | null>(null);
  const sim = MOCK_SIMULATION;
  const plans: [string, any][] = [["A", sim.A], ["B", sim.B]];
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl">
            ...
            {plans.map(([k, p]) => (
              <Card key={k} data-testid={`sim-plan-${k}`} className={`p-4 ${choice === k ? "border-primary" : ""}`}>
                ...
                <ul className="mt-2 space-y-1 text-sm">
                  {p.bullets.map((b: string, i: number) => <li key={i} className="flex gap-2"><Sparkles className="mt-0.5 h-3 w-3 shrink-0 text-accent" />{b}</li>)}
                </ul>
                <dl className="mt-3 divide-y divide-border rounded-xl border border-border">
                  {p.metrics.map(([label, value]: [string, string], i: number) => (
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)}>Cancelar</Button>
          <Button data-testid="sim-accept" disabled={!choice} onClick={() => { if (choice) onAccept?.(choice); onOpenChange(false); }}>Aceitar {choice ? `Plano ${choice}` : ""}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export function TroubleSaveDialog({ open, onOpenChange, onConfirm }: { open: boolean; onOpenChange: (o: boolean) => void; onConfirm?: (choice: string) => void }) {
  const [choice, setChoice] = useState<string | null>(null);
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <DialogFooter>
          ...
          <Button data-testid="trouble-save-confirm" disabled={!choice} onClick={() => { if (choice) onConfirm?.(choice); onOpenChange(false); }}>Confirmar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export function TroubleContent({ onFinishOffer }: { onFinishOffer?: () => void }) {
