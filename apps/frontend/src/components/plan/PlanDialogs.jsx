...
function whyForDay(recipe, plan, day) {
  ...
  const usedRecent = plan.some(p => p.recipeId === recipe.id && Math.abs(p.day - day) <= 2);
...
function WhyPill({ label, tone = "primary" }) {
...
/* ---------- Meal Picker ---------- */
export function MealPickerDialog({ open, onOpenChange, slot, recipes, plan, addPlan, tr }) {
  const [q, setQ] = useState("");
  const [selected, setSelected] = useState([]); // for compare mode
  ...
  const pick = (recipeId, state = "accepted") => {
    if (!slot) return;
...
const REPLACE_REASONS = [
  { key: "faster",   filter: (r, cur) => r.totalTime <= (cur?.totalTime || 60) - 5 },
  { key: "easier",   filter: (r, cur) => r.effort <= (cur?.effort || 3) - 1 || r.cleanup <= 1 },
  { key: "noMood",   filter: () => true },
  { key: "manyMiss", filter: (r) => r.missing.length <= 1 },
  { key: "haveAll",  filter: (r) => r.missing.length === 0 },
  { key: "different",filter: () => true },
];

export function MealReplaceDialog({ open, onOpenChange, entry, recipes, replaceMeal, tr }) {
  const [reasonKey, setReasonKey] = useState(null);
  const [pick, setPick] = useState(null);
  const [shoppingChoice, setShoppingChoice] = useState("keep");
  const reasonLabels = tr("plan.replace.reasons");
  const current = entry ? recipes.find(r => r.id === entry.recipeId) : null;

  const alternatives = useMemo(() => {
    if (!entry) return [];
    const list = recipes.filter(r => r.id !== entry.recipeId);
    const reason = REPLACE_REASONS.find(r => r.key === reasonKey);
    const filtered = reason ? list.filter(r => reason.filter(r, current)) : list;
...
/* ---------- Move Dialog ---------- */
export function MealMoveDialog({ open, onOpenChange, entry, plan, moveMeal, tr, recipes }) {
  ...
  const occupying = plan.find(p => p.id !== entry.id && p.day === dstDay && p.meal === dstMeal);
  const occRecipe = occupying ? recipes.find(r => r.id === occupying.recipeId) : null;
            ...
            <RadioGroup value={resolution} onValueChange={setResolution} className="mt-3 grid grid-cols-1 gap-1.5">
              {[
                ...
                <label key={k} className="flex items-start gap-2 text-sm">
                  <RadioGroupItem value={k} data-testid={`move-res-${k}`} className="mt-0.5" />
                  <span className="flex-1">
                    {tr(tk)}
                    {hintKey && <span className="block text-[11px] text-muted-foreground">{tr(hintKey)}</span>}
                  </span>
                </label>
              ))}
            </RadioGroup>
...
/* ---------- Mark Done Dialog ---------- */
export function MarkDoneDialog({ open, onOpenChange, entry, recipes, onConfirm, tr }) {
  if (!entry) return null;
...
export function MissingReviewDialog({ open, onOpenChange, plan, recipes, shopping, sendItemsToShopping, tr }) {
  const [groupBy, setGroupBy] = useState("meal");
  const [selection, setSelection] = useState({}); // key -> boolean
  const [qty, setQty] = useState({}); // key -> number
  ...
  const inShopping = useMemo(() => new Set(shopping.map(s => `${(s.name || "").toLowerCase()}|${(s.unit || "").toLowerCase()}`)), [shopping]);
  ...
  const selectAll = () => setSelection(items.reduce((a, it) => { a[it.key] = true; return a; }, {}));
...
/* ---------- Prep-Ahead Dialog ---------- */
const SLOT_ORDER = { "noite anterior": 0, "manhã": 1, "1h antes": 2 };
const SLOT_META = {
...
function taskStateFor(scenario, index, done) {
  if (done) return "done";
...
function TaskCard({ task, taskState, onMarkDone, onStart, tr, k }) {
...
function TrailView({ items, doneKeys, onMarkDone, onStart, tr, scenario, todayIdx }) {
  const groups = useMemo(() => {
    const g = { today: [], tomorrow: [], later: [] };
    items.forEach((it, i) => {
    ...
    Object.keys(g).forEach(key => g[key].sort((a, b) => (SLOT_ORDER[a.when] ?? 9) - (SLOT_ORDER[b.when] ?? 9)));
    return g;
  }, [items, todayIdx]);
...
export function PrepAheadDialog({ open, onOpenChange, prepItems, tr, onStart, onMarkDone, doneKeys, scenario, todayIdx }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
