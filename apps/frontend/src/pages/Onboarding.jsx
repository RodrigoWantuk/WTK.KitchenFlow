...
export default function Onboarding() {
  ...
  const steps = tr("onboarding.steps");
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={key}>
                  ...
                  <div className="mt-2 flex items-center gap-4">
                    <Slider data-testid={`ob-${key}`} value={[local[key]]} min={1} max={3} step={1} onValueChange={([v]) => setLocal({ ...local, [key]: v })} className="flex-1" />
                    <span className="w-16 text-center text-xs text-muted-foreground">{["Baixo","Médio","Alto"][local[key]-1]}</span>
                  </div>
                </div>
              ))}
            </div>
