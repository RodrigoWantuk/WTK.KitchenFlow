...
/* ---------- Weekly Summary Bar ---------- */
function SummaryBar({ plan, tr, onReviewMissing, onOpenPrep, prepPendingCount }) {
  const decided = plan.filter(p => p.state === "accepted").length;
  const suggested = plan.filter(p => p.state === "suggested").length;
  const drafts = plan.filter(p => p.state === "draft").length;
  const done = plan.filter(p => p.state === "done").length;
  const needsShopping = plan.filter(p => p.state === "needsShopping").length;
  const needsPrep = plan.filter(p => p.state === "needsPrep").length;
  const totalSlots = 7 * 2; // lunch + dinner as core slots
  const emptyCore = Math.max(0, totalSlots - plan.filter(p => p.meal === "lunch" || p.meal === "dinner").length);

  const chip = (label, n, tone = "default") => (
...
/* ---------- Cell (desktop grid) ---------- */
function MealCell({ entries, recipes, day, meal, onOpenPicker, onAction, tr }) {
  ...
  return (
    <div data-testid={`plan-cell-${day}-${meal}`} className="space-y-1.5">
      {entries.map(entry => {
        ...
        const replacedTitle = entry._replacedFrom ? (recipes.find(x => x.id === entry._replacedFrom)?.title) : null;
        return (
...
/* ---------- Action Menu ---------- */
function MealMenu({ entry, onAction, tr }) {
  return (
    <DropdownMenu>
...
/* ---------- Mobile Row ---------- */
function MealRow({ entry, recipes, tr, onAction }) {
  const r = recipes.find(x => x.id === entry.recipeId);
  const s = STATE_STYLE[entry.state] || STATE_STYLE.draft;
  const replacedTitle = entry._replacedFrom ? (recipes.find(x => x.id === entry._replacedFrom)?.title) : null;
...
export default function Plan() {
  ...
  const [picker, setPicker] = useState({ open: false, slot: null });
  const [replace, setReplace] = useState({ open: false, entry: null });
  const [move, setMove] = useState({ open: false, entry: null });
  const [missing, setMissing] = useState(false);
  const [prep, setPrep] = useState(false);
  const [prepDone, setPrepDone] = useState(new Set());
  const [activeDay, setActiveDay] = useState(0);
  const [markDone, setMarkDone] = useState({ open: false, entry: null });
  ...
  const prepItems = useMemo(() => {
    ...
    plan.forEach(p => {
      ...
      list.forEach(pa => out.push({ ...pa, planId: p.id, recipeId: p.recipeId, recipeTitle: recipes.find(r => r.id === p.recipeId)?.title, day: p.day, meal: p.meal }));
    });
    return out;
  }, [plan, recipes]);
  ...
  const handleMarkDoneConfirm = (mode) => {
  ...
  return (
    <div className="space-y-6">
              ...
              {[0,1,2,3,4,5,6].map(day => {
                ...
                return (
                  <MealCell
                    ...
                    day={day}
                    meal={meal}
                    onOpenPicker={(slot) => setPicker({ open: true, slot })}
              ...
              {["breakfast","snack"].map(meal => (
                <div key={meal} className="contents">
                  ...
                  {[0,1,2,3,4,5,6].map(day => {
                    ...
                    return <MealCell key={`${day}-${meal}`} entries={entries} recipes={recipes} day={day} meal={meal} onOpenPicker={(slot) => setPicker({ open: true, slot })} onAction={handleAction} tr={tr} />;
                  })}
                </div>
              ))}
            </div>
      ...
      <MealReplaceDialog
        open={replace.open}
        onOpenChange={(o) => setReplace(r => ({ ...r, open: o }))}
        entry={replace.entry}
        recipes={recipes}
      ...
      <PrepAheadDialog
        ...
        prepItems={prepItems}
        doneKeys={prepDone}
        onMarkDone={(k) => setPrepDone(s => new Set(s).add(k))}
      ...
      <MarkDoneDialog
        open={markDone.open}
        onOpenChange={(o) => setMarkDone(m => ({ ...m, open: o }))}
      ...
      <SimulationDialog open={simOpen} onOpenChange={setSimOpen} onAccept={(k) => toast.success(`Plano ${k} aceito`)} />
    </div>
  );
}
