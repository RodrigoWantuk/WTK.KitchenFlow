...
export default function Shopping() {
  ...
  const { shoppingRequirementProjections, enablePrototypeFixtures } =
    useRuntime();
  const [searchParams] = useSearchParams();
  const showRequirementReview =
    searchParams.get("review") === "shortfall" ||
    ...
    <div className="space-y-6">
      ...
      {showRequirementReview && (
        <ShoppingRequirementReview
          tr={tr}
          projections={shoppingRequirementProjections}
          showUnavailableWhenEmpty={!enablePrototypeFixtures}
          ...
          {AISLES.filter((a) => grouped[a]?.length).map((a) => (
            <Card key={a} className="p-4">
              <p className="mb-3 font-display text-lg">
                {tr(`shopping.aisles.${a}`)}
              </p>
              <ul className="divide-y divide-border">
                {grouped[a].map((i: (typeof shopping)[number]) => (
                  <li
                    key={i.id}
                    data-testid={`sh-item-${i.id}`}
                    className="flex items-center gap-3 py-3"
                  >
                    <Checkbox
                      data-testid={`sh-check-${i.id}`}
                      checked={i.checked}
                      onCheckedChange={() => toggleShopping(i.id)}
                      className="h-5 w-5"
                    />
                    <div className="min-w-0 flex-1">
                      <p
                        className={`text-sm ${i.checked ? "line-through text-muted-foreground" : ""}`}
                      >
                        {i.name}
                      </p>
                      <p className="text-[11px] text-muted-foreground">
                        {i.qty} {i.unit} · {tr(`shopping.origin.${i.origin}`)}
                      </p>
                    </div>
                    <button
                      data-testid={`sh-remove-${i.id}`}
                      onClick={() => removeShopping(i.id)}
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{tr("shopping.addItem")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <Input
              data-testid="sh-new-name"
              placeholder="Nome"
              value={newItem.name}
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">
              {tr("shoppingReconcile.title")}
            </DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            {tr("shoppingReconcile.description")}
          </p>
          <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-2xl border border-primary/30 bg-primary/5 p-3">
              <p
                className="font-display text-2xl leading-none"
                data-testid="reconcile-tomove"
              >
                {done}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {tr("shoppingReconcile.willAdd")}
              </p>
            </div>
            <div className="rounded-2xl border border-border bg-secondary/40 p-3">
              <p
                className="font-display text-2xl leading-none"
                data-testid="reconcile-tokeep"
              >
                {shopping.length - done}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {tr("shoppingReconcile.willKeep")}
              </p>
            </div>
          </div>
          {done > 0 && (
            <ul className="mt-3 max-h-40 space-y-1 overflow-y-auto rounded-xl border border-border bg-card p-2 text-xs">
              {shopping
                ...
                .map((i) => (
                  <li
                    key={i.id}
                    data-testid={`reconcile-item-${i.id}`}
          ...
          <div className="mt-3 flex justify-end gap-2">
            <Button variant="ghost" onClick={() => setFinish(false)}>
              {tr("common.cancel")}
            </Button>
            <Button
              data-testid="sh-finish-confirm"
              disabled={done === 0}
              onClick={() => {
                finishShopping();
                setFinish(false);
