...
export default function CookFlow() {
  ...
  const initialPhase =
    searchParams.get("phase") === "finish" ? "finish" : "prep";
  ...
  const showTrouble =
    scenario === "troubleSuccess" || scenario === "derivedRecipe";
  const showFullReconcile =
    scenario === "fullReconcile" ||
    scenario === "withLeftovers" ||
    scenario === "frozenPortions";

  useEffect(() => {
    if (!cooking && phase === "cook" && id) startCooking(id);
      ...
      >
        ...
        {sourcePreparationRouteId && (
          <p
            ...
            data-testid="cook-handoff-source"
          >
        ...
        <Card className="p-5">
          <ul className="space-y-3">
            {[
              ...
              "Medir quantidades",
              "Pré-aquecer se necessário",
              "Descongelar (se aplicável)",
            ].map((s, i) => (
              <li key={i} className="flex items-center gap-3">
                <Checkbox
                  data-testid={`prep-check-${i}`}
                  checked={!!prep[i]}
                  onCheckedChange={(v) => setPrep({ ...prep, [i]: v === true })}
                />
                <span
                  className={
                    prep[i] ? "line-through text-muted-foreground" : ""
                  }
                >
                  {s}
                </span>
        ...
        <Button
          data-testid="prep-start-cook"
          className="w-full rounded-full"
          onClick={() => setPhase("cook")}
        >
          {tr("cook.cook")} <ArrowRight className="ml-1 h-4 w-4" />
        </Button>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur">
          <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-4">
            <button
              data-testid="cook-exit"
              onClick={() => nav("/app/hoje")}
              className="flex items-center gap-1 text-sm text-muted-foreground"
            >
              <X className="h-4 w-4" /> {tr("cook.exit")}
            </button>
            <p className="font-display text-sm">
              {tr("cook.step")} {step + 1} {tr("cook.of")} {total}
            </p>
            <button
              data-testid="cook-help"
        ...
        <main className="mx-auto max-w-3xl px-6 py-10 md:py-14">
          ...
          <div className="mt-8 rounded-2xl border border-border bg-card p-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display text-2xl">{fmt(timer)}</span>
              </div>
              <div className="flex gap-2">
                ...
                >
                  {running ? tr("cook.stop") : tr("cook.start")}
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 flex items-center justify-between gap-2">
            <Button
              ...
              disabled={step === 0}
              onClick={() => updateCookStep(step - 1)}
            >
              <ChevronLeft className="h-4 w-4" /> {tr("cook.prev")}
            </Button>
            <Button
              variant="ghost"
              data-testid="cook-pause"
              onClick={() => (paused ? resumeCooking() : pauseCooking())}
            >
              {paused ? (
            ...
            {step < total - 1 ? (
              <Button
                ...
                onClick={() => updateCookStep(step + 1)}
              >
                {tr("cook.next")} <ChevronRight className="h-4 w-4" />
              </Button>
            ) : (
              <Button
        ...
        <Dialog open={helpOpen} onOpenChange={setHelp}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{tr("cook.helpTitle")}</DialogTitle>
            </DialogHeader>
            {showTrouble ? (
              <TroubleContent
                onFinishOffer={() => {
                  setHelp(false);
                  setTroubleSaveOpen(true);
                }}
              />
            ) : (
              <ul className="space-y-3 text-sm">
                {r.troubleshooting.map((h, i) => (
                  <li key={i} className="rounded-xl bg-secondary/60 p-3">
                    ...
                    <p className="mt-1 text-muted-foreground">{h.a}</p>
                  </li>
                ))}
                {r.troubleshooting.length === 0 && (
                  <li className="text-muted-foreground">
                    Sem sugestões específicas para essa receita.
                  </li>
                )}
              </ul>
            )}
        ...
        <TroubleSaveDialog
          open={troubleSaveOpen}
          onOpenChange={setTroubleSaveOpen}
          onConfirm={(k: string) =>
            toast.success(
              k === "derived"
                ? "Receita derivada criada"
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Vamos atualizar sua despensa sinteticamente.
      </p>
      {showFullReconcile && <ReconciliationTable />}
      <Card className="grid gap-4 p-6 sm:grid-cols-2">
        {[
          ["served", "cook.served"],
          ["leftovers", "cook.leftovers"],
          ["refrigerated", "cook.refrigerated"],
          ["frozen", "cook.frozen"],
          ["discarded", "cook.discarded"],
        ].map(([k, label]) => (
          <div key={k}>
            <label className="text-sm">{tr(label)}</label>
            <Input
              data-testid={`fin-${k}`}
              value={rec[k as keyof typeof rec]}
