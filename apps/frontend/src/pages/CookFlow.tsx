...
export default function CookFlow() {
  ...
  useEffect(() => { if (!cooking && phase === "cook" && id) startCooking(id); }, [phase]); // eslint-disable-line
  useEffect(() => {
    if (!running) return;
          ...
          <ul className="space-y-3">
            {["Separar ingredientes","Conferir equipamentos","Medir quantidades","Pré-aquecer se necessário","Descongelar (se aplicável)"].map((s, i) => (
              <li key={i} className="flex items-center gap-3">
                <Checkbox data-testid={`prep-check-${i}`} checked={!!prep[i]} onCheckedChange={(v) => setPrep({ ...prep, [i]: v === true })} />
  ...
  if (phase === "cook") {
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <TroubleSaveDialog open={troubleSaveOpen} onOpenChange={setTroubleSaveOpen} onConfirm={(k: string) => toast.success(k === "derived" ? "Receita derivada criada" : "Aprendizado registrado")} />
      </div>
    );
  }
      ...
      <Card className="grid gap-4 p-6 sm:grid-cols-2">
        {[["served","cook.served"],["leftovers","cook.leftovers"],["refrigerated","cook.refrigerated"],["frozen","cook.frozen"],["discarded","cook.discarded"]].map(([k, label]) => (
          <div key={k}>
            ...
            <Input data-testid={`fin-${k}`} value={rec[k as keyof typeof rec]} onChange={(e) => setRec({ ...rec, [k]: e.target.value })} placeholder="ex.: 2 porções" />
          </div>
        ))}
