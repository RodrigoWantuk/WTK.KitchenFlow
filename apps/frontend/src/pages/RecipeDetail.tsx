...
import { ChefHat, Sparkles, Check, X, ArrowLeft } from "lucide-react";
import { toast } from "sonner";
import {
  RecipeComponentDependency,
...
export default function RecipeDetail() {
  ...
  const showComponentDep =
    r.id === "r3" ||
    scenario === "componentScheduled" ||
    scenario === "componentShared" ||
    scenario === "componentInPantry";

  return (
    <div className="space-y-6">
      <button
        onClick={() => nav(-1)}
        data-testid="rec-back"
        ...
        <div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Card className="p-3">
              ...
              <p className="font-display text-lg">{r.totalTime} min</p>
            </Card>
            <Card className="p-3">
              <p className="text-xs text-muted-foreground">
                {tr("recipes.servings")}
              </p>
              <p className="font-display text-lg">{r.servings}</p>
            </Card>
            <Card className="p-3">
              <p className="text-xs text-muted-foreground">
                {tr("recipes.difficulty")}
              </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button
              ...
              variant="outline"
              onClick={() => {
                addMissingToShopping(r.id);
                toast.success("Faltantes adicionados às compras");
              }}
            >
              {tr("recipes.addMissingToShopping")}
            </Button>
            <Button
              data-testid="rec-add-plan"
              variant="outline"
              onClick={() => {
                addPlan({
                  day: 0,
                ...
                toast.success("Adicionada ao planejamento");
              }}
            >
              {tr("recipes.addToPlan")}
            </Button>
            <Button
              data-testid="rec-toggle-fav"
              variant="ghost"
              onClick={() => toggleFavorite(r.id)}
            >
              {favorites.includes(r.id) ? "★ Favorito" : "☆ Favoritar"}
            </Button>
          </div>
        </div>
      </div>
      ...
      {showComponentDep && (
        <>
          ...
          <ScheduleComponentDialog
            ...
            onConfirm={() => toast.success("Preparo agendado")}
          />
        </>
      )}
      ...
      <div className="grid gap-6 md:grid-cols-2">
            ...
            {r.ingredients.map((i, k) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-xs ${has ? "text-primary" : "text-warning"}`}
                  >
                    {has ? "Tem" : "Falta"}
                  </span>
                </li>
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {tr("recipes.equipment")}: {r.equipment.join(", ")}
          </div>
        </Card>
      </div>
    </div>
