...
import {
  ...
  SimulationDialog,
  RouteChainView,
} from "@/components/plan/PlanExtras";
...
> = {
  ...
  suggested: {
    icon: Sparkles,
    cls: "border-dashed border-accent/40 bg-accent/5",
    dot: "bg-accent",
  },
  draft: {
    icon: PenLine,
    cls: "border-dashed border-border bg-card",
    dot: "bg-muted-foreground",
  },
  incomplete: {
    icon: AlertTriangle,
    cls: "border-warning/60 bg-warning/10",
    dot: "bg-warning",
  },
  needsShopping: {
    icon: ShoppingBag,
    cls: "border-warning/50 bg-warning/5",
    dot: "bg-warning",
  },
  needsPrep: {
    icon: Flame,
  ...
  skipped: {
    icon: X,
    cls: "border-border bg-muted/40 opacity-80",
    dot: "bg-muted-foreground",
  },
};
  ...
  return (
    <div className="flex flex-wrap gap-1">
      ...
      {entry._reconciliationPending && (
        <span
          data-testid={`reconcile-pending-${entry.id}`}
          className="inline-flex items-center gap-1 rounded-full bg-warning/20 px-1.5 py-0.5 text-[10px] font-medium"
        >
...
function SummaryBar({
  ...
  const needsShopping = plan.filter(
    (p: any) => p.state === "needsShopping",
  ).length;
  const needsPrep = plan.filter((p: any) => p.state === "needsPrep").length;
  const totalSlots = 7 * 2; // lunch + dinner as core slots
  const emptyCore = Math.max(
    0,
    totalSlots -
      plan.filter((p: any) => p.meal === "lunch" || p.meal === "dinner").length,
  );

  const chip = (label: string, n: number, tone = "default") => (
    <div
      className={`rounded-xl border px-3 py-2 ${tone === "warn" ? "border-warning/40 bg-warning/10" : tone === "primary" ? "border-primary/40 bg-primary/5" : "border-border bg-card"}`}
    >
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Planejar é intenção, não obrigação.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            size="sm"
            variant="outline"
            data-testid="summary-open-prep"
            onClick={onOpenPrep}
            className="rounded-full"
          >
            <Flame className="mr-1 h-4 w-4" /> Preparo antecipado{" "}
            {prepPendingCount > 0 && (
              <span className="ml-1 rounded-full bg-accent/30 px-1.5 text-[10px]">
                {prepPendingCount}
              </span>
            )}
          </Button>
          <Button
            size="sm"
            data-testid="summary-review-missing"
...
function MealCell({
  ...
  day: number;
  meal: string;
  onOpenPicker: (slot: { day: number; meal: string }) => void;
  ...
  return (
    <div data-testid={`plan-cell-${day}-${meal}`} className="space-y-1.5">
      {entries.map((entry: any) => {
        ...
        const replacedTitle = entry._replacedFrom
          ? recipes.find((x: any) => x.id === entry._replacedFrom)?.title
          : null;
        return (
          <div
            key={entry.id}
            data-testid={`plan-entry-${entry.id}`}
            className={`min-h-[80px] rounded-xl border p-2 ${s.cls}`}
          >
            ...
            {r ? (
              <Link
                ...
                data-testid={`plan-open-${entry.id}`}
                className={`mt-0.5 block truncate font-medium leading-tight hover:underline ${entry.state === "skipped" ? "line-through text-muted-foreground" : ""}`}
              >
                {r.title}
              </Link>
            ) : (
            ...
            {replacedTitle && (
              <p
                data-testid={`plan-replaced-${entry.id}`}
                className="mt-0.5 text-[10px] text-muted-foreground italic"
              >
                {tr("plan.replacedFrom")} "{replacedTitle}"
              </p>
            )}
            <div className="mt-1">
              <StatePill entry={entry} tr={tr} />
            </div>
            <p className="mt-1 line-clamp-2 text-[11px] text-muted-foreground">
              {tr(`plan.stateHints.${entry.state}`)}
            </p>
          </div>
        );
      })}
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          data-testid={`plan-menu-${entry.id}`}
      ...
      <DropdownMenuContent align="end" className="w-56">
        {entry.state === "suggested" && (
          <DropdownMenuItem
            data-testid={`plan-action-accept-${entry.id}`}
            onClick={() => onAction("accept", entry)}
          >
            <Check className="mr-2 h-4 w-4" />
            {tr("plan.actions.accept")}
          </DropdownMenuItem>
        )}
        <DropdownMenuItem
          data-testid={`plan-action-open-${entry.id}`}
          onClick={() => onAction("open", entry)}
        >
          <ArrowRight className="mr-2 h-4 w-4" />
          {tr("plan.actions.open")}
        </DropdownMenuItem>
        <DropdownMenuItem
          data-testid={`plan-action-replace-${entry.id}`}
          onClick={() => onAction("replace", entry)}
        >
          <Sparkles className="mr-2 h-4 w-4" />
          {tr("plan.actions.replace")}
        </DropdownMenuItem>
        <DropdownMenuItem
          data-testid={`plan-action-move-${entry.id}`}
          onClick={() => onAction("move", entry)}
        >
          <CalendarClock className="mr-2 h-4 w-4" />
          {tr("plan.actions.move")}
        </DropdownMenuItem>
        <DropdownMenuItem
          data-testid={`plan-action-todraft-${entry.id}`}
          onClick={() => onAction("toDraft", entry)}
        >
          <PenLine className="mr-2 h-4 w-4" />
          {tr("plan.actions.toDraft")}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem
          data-testid={`plan-action-addmissing-${entry.id}`}
          onClick={() => onAction("addMissing", entry)}
        >
          <ShoppingBag className="mr-2 h-4 w-4" />
          {tr("plan.actions.addMissing")}
        </DropdownMenuItem>
        <DropdownMenuItem
          data-testid={`plan-action-startprep-${entry.id}`}
          onClick={() => onAction("startPrep", entry)}
        >
          <ChefHat className="mr-2 h-4 w-4" />
          {tr("plan.actions.startPrep")}
        </DropdownMenuItem>
        {entry.state !== "done" && (
          <DropdownMenuItem
            data-testid={`plan-action-markdone-${entry.id}`}
            onClick={() => onAction("markDone", entry)}
          >
            <Check className="mr-2 h-4 w-4" />
            {tr("plan.actions.markDone")}
          </DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem
          data-testid={`plan-action-remove-${entry.id}`}
          onClick={() => onAction("remove", entry)}
          className="text-destructive"
        >
          <X className="mr-2 h-4 w-4" />
          {tr("plan.actions.remove")}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
function MealRow({
  ...
  const primary =
    entry.state === "suggested"
      ? { key: "accept", label: tr("plan.actions.accept") }
      : entry.state === "needsShopping"
        ? { key: "addMissing", label: tr("plan.actions.addMissing") }
        : entry.state === "needsPrep"
          ? { key: "startPrep", label: tr("plan.actions.startPrep") }
          : entry.state === "draft" || entry.state === "incomplete"
            ? { key: "replace", label: tr("plan.actions.replace") }
            : entry.state === "done"
              ? { key: "open", label: tr("plan.actions.open") }
              : entry.state === "skipped"
                ? { key: "replace", label: tr("plan.actions.replace") }
  ...
  return (
    <div
      ...
      className={`rounded-2xl border p-3 ${s.cls}`}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p
            className={`truncate text-sm font-medium ${entry.state === "skipped" ? "line-through text-muted-foreground" : ""}`}
          >
            {r?.title || "Sem receita"}
          </p>
          {replacedTitle && (
            <p
              data-testid={`plan-row-replaced-${entry.id}`}
      ...
      <div className="mt-3 flex gap-2">
        <Button
          size="sm"
          data-testid={`plan-primary-${entry.id}`}
          onClick={() => onAction(primary.key, entry)}
        >
          {primary.label}
        </Button>
        <Button
          size="sm"
          variant="outline"
          data-testid={`plan-secondary-${entry.id}`}
          onClick={() => onAction("open", entry)}
        >
          {tr("plan.actions.open")}
        </Button>
...
export default function Plan() {
  ...
  const [replace, setReplace] = useState<{ open: boolean; entry: any | null }>({
    open: false,
    entry: null,
  });
  const [move, setMove] = useState<{ open: boolean; entry: any | null }>({
    open: false,
    entry: null,
  });
  ...
  const handleAction = (action: string, entry: any) => {
    switch (action) {
      case "open":
        navigate(`/app/receitas/${entry.recipeId}`);
        break;
      case "accept":
        ...
        break;
      case "replace":
        setReplace({ open: true, entry });
        break;
      case "move":
        setMove({ open: true, entry });
        break;
      case "toDraft":
        setMealState(entry.id, "draft");
        toast("Movido para rascunho");
        break;
      case "remove":
        removePlan(entry.id);
  ...
  const handleMarkDoneConfirm = (mode: string) => {
    ...
    if (mode === "review") {
      ...
      navigate(
        `/app/cozinhar/${entry.recipeId}?phase=finish&planId=${entry.id}`,
      );
      return;
    }
    ...
    setMarkDone({ open: false, entry: null });
    toast.success(
      mode === "skipped" ? tr("plan.markedSkipped") : tr("plan.markedDone"),
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          variant="outline"
          data-testid="open-reality-changed"
        ...
        <Button
          variant="outline"
          data-testid="open-week-simulation"
          onClick={() => setSimOpen(true)}
        >
          Simular semana (A vs B)
        </Button>
        <Button
          variant="outline"
          data-testid="open-schedule-component"
          onClick={() => setScheduleOpen(true)}
        >
          Agendar preparo de componente
        </Button>
      </div>

      {showRouteChain && (
        <section
          data-testid="plan-route-chain-section"
          className="rounded-3xl border border-border bg-secondary/30 p-4 md:p-5"
        >
          <div className="mb-3 flex items-baseline justify-between">
            <div>
              <p className="font-display text-lg">Rota com dependências</p>
              <p className="text-xs text-muted-foreground">
                Uma preparação libera as próximas quando concluída.
              </p>
            </div>
          </div>
          <RouteChainView />
        ...
        <div className="grid grid-cols-[80px_repeat(7,1fr)] gap-2">
          ...
          {["lunch", "dinner"].map((meal) => (
            <div key={meal} className="contents">
              <div className="flex items-center justify-end pr-2 text-xs uppercase tracking-wider text-muted-foreground">
                {tr(`plan.${meal}`)}
              </div>
              {[0, 1, 2, 3, 4, 5, 6].map((day) => {
                const entries = plan.filter(
                  (p) => p.day === day && p.meal === meal,
                );
                return (
                  <MealCell
                    key={`${day}-${meal}`}
                    ...
                    day={day}
                    meal={meal}
                    onOpenPicker={(slot: { day: number; meal: string }) =>
                      setPicker({ open: true, slot })
                    }
        ...
        <div className="sticky top-16 z-30 -mx-4 flex gap-1.5 overflow-x-auto border-b border-border bg-background/95 px-4 py-2 backdrop-blur">
          {DAY_SHORT.map((d, i) => {
            ...
            return (
              <button
                key={i}
                data-testid={`plan-day-tab-${i}`}
                onClick={() => setActiveDay(i)}
                className={`flex flex-col items-center rounded-xl px-3 py-1.5 text-xs ${active ? "bg-primary text-primary-foreground" : "bg-secondary/50 text-muted-foreground"}`}
              >
                <span className="font-display">{d}</span>
                <span
                  className="mt-0.5 h-1.5 w-1.5 rounded-full"
                  style={{
                    background: dayPlan.length ? "currentColor" : "transparent",
                    opacity: 0.6,
                  }}
                />
              </button>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {plan.filter((p) => p.day === activeDay).length} refeições
            </span>
          </div>
          {MEALS.map((meal) => {
            const entries = plan.filter(
              (p) => p.day === activeDay && p.meal === meal,
            );
            if (!entries.length) {
              return (
                <button
                  ...
                  onClick={() =>
                    setPicker({ open: true, slot: { day: activeDay, meal } })
                  }
                ...
                >
                  <div>
                    ...
                    <p className="mt-0.5 text-sm text-muted-foreground">
                      {tr("plan.empty")} — {tr("plan.emptyHint")}
                    </p>
                  </div>
      ...
      <ScheduleComponentDialog
        ...
        onConfirm={() =>
          toast.success("Preparo agendado — componente aparecerá na despensa")
        }
      />
      <SimulationDialog
        open={simOpen}
        onOpenChange={setSimOpen}
        onAccept={(k: string) => toast.success(`Plano ${k} aceito`)}
      />
    </div>
  );
}
