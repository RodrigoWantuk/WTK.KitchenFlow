...
public static class InventoryEndpoints
{
    ...
    public static RouteGroupBuilder MapInventoryEndpoints(this RouteGroupBuilder group)
    {
        ...
        group.MapGet("/lots", (InventoryApplicationService service, int? pageSize, string? status, string? storageLocation, string? search, string? cursor, CancellationToken cancellationToken) => service.ListAsync(pageSize, status, storageLocation, search, cursor, cancellationToken)).Produces<ListLotsResponse>().ProducesProblem(400);
        ...
        group.MapGet("/lots/{lotId:guid}", (InventoryApplicationService service, Guid lotId, CancellationToken cancellationToken) => service.GetAsync(lotId, cancellationToken)).Produces<LotResponse>().ProducesProblem(404);
        ...
        group.MapGet("/lots/{lotId:guid}/history", (InventoryApplicationService service, Guid lotId, CancellationToken cancellationToken) => service.HistoryAsync(lotId, cancellationToken)).Produces<IReadOnlyList<LotHistoryResponse>>().ProducesProblem(404);
        return group;
    }
    ...
    private static async ValueTask<object?> ValidateCsrfAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        if (HttpMethods.IsPost(context.HttpContext.Request.Method) || HttpMethods.IsPatch(context.HttpContext.Request.Method) || HttpMethods.IsDelete(context.HttpContext.Request.Method))
        {
            ...
            {
                return Results.Problem(statusCode: StatusCodes.Status400BadRequest, extensions: new Dictionary<string, object?> { ["errorCode"] = "validation_failed", ["traceId"] = context.HttpContext.TraceIdentifier });
            }
        }
